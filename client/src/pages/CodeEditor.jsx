...
const CodeEditor = () => {
    ...
    const getRepo = async () => {
        try {
            const { data } = await axios.get(`${Host}/api/repo/get-Repo/${repoId}`);
            setName(data?.Repo.name)
            setHtmlCode(data?.Repo.HTML)
            setCssCode(data?.Repo.CSS)
            setJsCode(data?.Repo.JS)
        } catch (error) {
            console.log(error)
        }
    ...
    const handleSaveCode = async () => {
        try {
            const { data } = await axios.put(`${Host}/api/repo/update-Repo/${repoId}`, {
    ...
    return (
        <>
            <div className='flex flex-col h-screen'>
                <div className='h-[10%] w-screen flex flex-row bg-gray-800 relative'>
                    <div className='flex flex-row'>
                        ...
                        <h1 className=' font-serif font-bold mt-3 ms-4 text-white'>{name}</h1>
                    </div>
                    {repoId ? (<button
                        className="bg-green-500 hover:bg-green-700 text-white font-bold px-5 m-2 rounded"
                        onClick={handleSaveCode}
                        aria-label="Save Code"
                    >
                        Save
                    </button>) : (
                        <div className='text-white '>Login to save your code!</div>
                    )
            ...
            {consoleVisible && (
                <div className="bg-gray-800 border-gray-100 p-4 w-[70%] h-[40%] absolute bottom-0 right-0 ">
